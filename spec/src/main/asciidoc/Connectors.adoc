...
<<a9745, Java Platform, Standard Edition 7APISpecification>>)
...
server in <<a5151, Class Loading Requirements>>.
...
adapter. See <<a5151, Class Loading Requirements>>,
for more information on when an application may portably
...
<<a9753, Bean Validation Specification, version 1.1>>)
to a resource adapter JavaBean, as detailed in
<<a516, JavaBean Validation>>, the existing
...
element, described in <<a2738, ActivationSpec JavaBean>>,
is deprecated since the Java EE Connector Architecture 1.6
...
annotations (see <<a4517, Metadata Annotations>>), it is simpler to develop a resource adapter module. By
...
_endpointDeactivation_ in <<a2765, Application Server>>

Added requirements in
<<a2598, Message Inflow Model>> for the
...
Added a new section,
<<a4814, Resource Definition Annotations>>, to
...
_ActivationSpec_ interface (See <<a4758, @Activation>>)

Removed Appendix B “Activation Configuration
for Message Inflow to JMS Endpoints”. Chapter 12 of the
<<a9743, Java Message Service Specification, version 2.0>>
defines the standard configuration elements for JMS resource adapters

{empty}Clarified the dependency injection
behavior when JavaBeans that are managed by the application server are
used as CDI Managed Beans in <<a6725, Dependency
Injection]

Updated <<a5430, Resource Adapter XML Schema Definition>> to reflect the new 1.7 resource
adapter deployment descriptor and added a new
<<a8321, Java EE Connector Architecture 1.6 Resource Adapter XML XSD>>
to house the 1.6 resource adapter deployment descriptor.
